...
export default function ChatInput({
  ...
  return (
    <form
      ...
      className={`overflow-hidden rounded-lg border transition-colors ${disabled ? 'cursor-not-allowed' : 'focus-within:border-white hover:border-white'}`}
      ...
      <div className="flex flex-col">
        ...
        <div className={`flex items-center justify-between px-2 py-1.5 ${disabled ? 'bg-gray-100' : 'bg-white'}`}>
          ...
          {showStopButton ? (
            <button
              type="button"
              onClick={onStopTask}
              className="rounded-md bg-white px-3 py-1 text-gray-900 transition-colors hover:bg-gray-200">
              Stop
            </button>
          ) : historicalSessionId ? (
            <button
